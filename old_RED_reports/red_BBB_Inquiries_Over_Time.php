...
if ($_POST) {
	if ($iPeriod == 'years') {
		$query = "
			...
			select
				sum(s.CountOfInquiries) as Inquiries,
				[Year]
		...
		$query = "
			...
			select
				sum(s.CountOfInquiries) as Inquiries,
				(CONVERT(varchar(7), CAST( [MonthNumber] as VARCHAR) + '/' + CAST( [Year] AS VARCHAR) )) AS InquiryMonthYearText
			from SnapshotStats s WITH (NOLOCK)
			inner join BBB WITH (NOLOCK) on BBB.BBBID = s.BBBID AND BBB.BBBBranchID = 0 AND BBB.IsActive = '1'
			where
				('" . $iBBBID . "' = '' or BBB.BBBID = '" . $iBBBID . "') and
	...
	if (count($rs) > 0 && $output_type == "") {
		...
		$barchart->DrawTitle('BBB Inquiries over Time');
