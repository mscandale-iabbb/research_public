...
if (! $iSortBy) $iSortBy = 'CustRevs DESC';
...
if ($_POST) {
	$query = "SELECT TOP " . $iMaxRecs . "
		...
		from BusinessCustomerReview c WITH (NOLOCK)
		...
		WHERE
			c.DateReceived >= '{$iDateFrom}' and c.DateReceived <= '{$iDateTo}' and
			c.IsPublished = '1' and
			len(b.TOBID) = 9 and
	...
	if (count($rs) > 0) {
		$report->WriteHeaderRow(
			array (
				array('#', '', '', 'right'),
				array('Published Customer Reviews', $SortFields['Customer Reviews'], '', 'right'),
