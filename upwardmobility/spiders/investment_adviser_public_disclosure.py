...
class InvestmentAdviserPublicDisclosureSpider(scrapy.Spider):
    name = 'investment_adviser_public_disclosure'
    custom_settings = {'CONCURRENT_REQUESTS': 1}
    allowed_domains = ['adviserinfo.sec.gov']
    start_urls = ['https://adviserinfo.sec.gov/']
    buf = []
    headers = {
        'authority': 'api.adviserinfo.sec.gov',
    ...
    def parse(self, response):
        for searchText in string.ascii_lowercase:
            u = f"https://api.adviserinfo.sec.gov/search/individual?query={searchText}&filter=active=true,prev=true,bar=true,broker=true,ia=true,brokeria=true&includePrevious=true&hl=true&nrows=12&start=0&r=25&sort=score+desc&wt=json"
    ...
    def get_data(self, response):
        ...
        for d in data:
            ...
            l.add_value('source', 'Investment Adviser Public Disclosure')
            ...
            for b in d['_source'].get('ind_ia_current_employments', []):
        ...
        if len(data) == 12:
            ...
            u = f"https://api.adviserinfo.sec.gov/search/individual?query={searchText}&filter=active=true,prev=true,bar=true,broker=true,ia=true,brokeria=true&includePrevious=true&hl=true&nrows=12&start={start+12}&r=25&sort=score+desc&wt=json"
